...
from Flow_Matching.models import UNet
...
def define_model(args):
    if args.model in ("ot", "gradient_step","fm_indep"):
        # use the local U-Net architecture
        model = BaseUNet(
            dim=args.dim,                             # e.g., 128
            dim_mults=tuple(args.dim_mults),          # e.g., (1,1,2,2,4,4)
            channels=args.num_channels,               # 3 for RGB
            dropout=args.dropout,                     # e.g., 0.0
            

        ).to(args.device)
        return (model, None)
    elif args.model == "fm_cond_s_he":
        model = BaseUNet(
            dim=args.dim,
            dim_mults=tuple(args.dim_mults),
            in_channels=3,      # [S_t, H, E]
            out_channels=1,     # predict S velocity (1 channel)
        ...
        raise NotImplementedError(
            "Rectified mode was removed from image_generation during HES-only cleanup."
        )
    else:
...
def save_samples(samples, train_samples, path, args):
    ...
    is_gray_out   = (samples.shape[-1] == 1)
    is_gray_train = (train_samples.shape[-1] == 1)

    # find cols = largest divisor ≤ sqrt(n)
    ...
    def get_ax(i, j):
        # when rows == 1, ax is 1D with length 2*cols
        return ax[i, j] if rows > 1 else ax[j]

    # Model generations (left side)
    for k in range(batch_samples_size):
        i = k // cols
        j = k % cols
        axi = get_ax(i, j)
        if is_gray_out:
            axi.imshow(samples[k].squeeze(-1), cmap='gray', vmin=0, vmax=1)
        else:
            axi.imshow(samples[k])

    # Training samples (right side)
    n_train = min(train_samples.shape[0], batch_samples_size)
    for k in range(n_train):
        i = k // cols
        j = k % cols
        col_idx = j + cols  # right side
        axi = get_ax(i, col_idx)
        if is_gray_train:
            axi.imshow(train_samples[k].squeeze(-1), cmap='gray', vmin=0, vmax=1)
        else:
            axi.imshow(train_samples[k])

    # Titles
    get_ax(0, 0).set_title("Model samples")
    get_ax(0, cols).set_title("Training samples")

    # Hide axes
    for axi in (ax.flatten() if rows > 1 else ax):
        axi.set_xticks([])
        axi.set_yticks([])
...
def postprocess(img, args):
    ...
    if args.model == "ot" or args.model == "gradient_step" or args.model == "diffusion":
        if args.dataset == "afhq_cat":
            ...
            img = invTrans(img)
        # for all flow models (classic, ot, gradient_step, diffusion, rectified…)
    if args.model in ("fm_indep","fm_cond_s_he","ot", "gradient_step", "diffusion", "rectified"):
        img = (img + 1.) / 2.
    return img
