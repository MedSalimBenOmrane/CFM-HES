...
from Flow_Matching.image_generation.models.utils import from_flattened_numpy, to_flattened_numpy, get_score_fn
from scipy import integrate
import Flow_Matching.image_generation.sde_lib as sde_lib
import Flow_Matching.image_generation.models.utils as mutils
...
def _sde_euler_maruyama_sampler(model, sde, shape, inverse_scaler, device):
    ...
    # 1) Initialize at t=1 with Gaussian noise
    #    -> use `noise_scale` (not `init_noise_scale`)
    x = torch.randn(shape, device=device) * sde.noise_scale
    nfe = 0

    # 2) Uniform time discretization from T=1 to T=0 in sample_N steps
    T = torch.linspace(1.0, 0.0, sde.sample_N, device=device)
    dt = (T[1] - T[0]).item()
    model_fn = mutils.get_model_fn(model, train=False)
    for t in T:
        t_vec = torch.full((batch_size,), t, device=device)
        # Compute velocity from the model
        v = model(x, t_vec).real
        nfe += 1
        # Euler-Maruyama step: x <- x + v*dt + sigma*sqrt(|dt|)*noise
        noise_term = torch.randn_like(x) * (sde.sigma_t(t) * (dt**0.5))
        x = x + v * dt + noise_term

    # 3) Rescale back to [0,1] or [-1,1]
    x = inverse_scaler(x)
    return x, nfe
